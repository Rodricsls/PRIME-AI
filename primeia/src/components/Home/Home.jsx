...
export default function Home() {
  ...
  async function getDayProgress(Email, day) {
    try {
      const response = await axios.post('https://primeai.azurewebsites.net/dayProgress', { correo: Email, dia: day }, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
  ...
  async function getRacha(Email) {
    try {
      const response = await axios.post('https://primeai.azurewebsites.net/streak', { correo: Email }, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
  ...
  async function getData(Email) {
    try {
      ...
      const response = await axios.post('https://primeai.azurewebsites.net/user', { correo: Email },{ headers:{Authorization:`Bearer ${token}`}});
      const data = response.data;
      if (data.resultado) {
        console.log(data);
  ...
  async function getWeekProgress(Email) {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post('https://primeai.azurewebsites.net/weekProgress', { correo: Email },{ headers:{Authorization:`Bearer ${token}`}});
  ...
  async function getTop3() {
    try {
      const token = localStorage.getItem('token');
      const result = await axios.post('https://primeai.azurewebsites.net/topThreeStreaks',{}, { headers:{Authorization:`Bearer ${token}`} });
      ...
      try {
        if (localStorage.getItem('token') === null || localStorage.getItem('email') === null) {
          ...
          const response = await axios.post(
            'https://primeai.azurewebsites.net/verificarToken',
            {},
