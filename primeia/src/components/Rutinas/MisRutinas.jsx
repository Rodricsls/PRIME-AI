...
export default function MisRutinas(props) {
  ...
  async function eliminarRutina(idr) {
    setOpen(false);
    try {
      const response = await axios.post('https://primeai-api.azurewebsites.net/DeleteRutina', { correo:correo ,id_rutina: idr }, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
